...
SELECT b.BuyerName, l.Finance_Type, l.LoanAmount, 
	   l.Loan_Term, (l.LoanAmount / l.Loan_Term) AS YearlyPayment
FROM profriends_inc.buyer b
JOIN profriends_inc.loan l
ON b.BuyerID = l.BuyerID
WHERE l.Loan_Term > 5
AND (l.LoanAmount / l.Loan_Term) > 200000;
...
SELECT b.BuyerName, COUNT(be.BeneficiaryID) AS TotalBeneficiaries,
	   MAX(be.BenF_Bdate) AS 'Youngest Beneficiary Bdate'
FROM profriends_inc.buyer b
JOIN profriends_inc.beneficiary be
ON b.BuyerID = be.BuyerID
GROUP BY b.BuyerName
HAVING COUNT(be.BeneficiaryID) > 1
